...
create table if not exists dm_aifox.asset_detail_jf as 
...
create table if not exists dm_aifox.d3level as 
  select
    asset_item_no,
...
create table if not exists dm_aifox.d1level as 
  select
    asset_item_no,
    date(create_time) as dt,
    due_diff_days,
...
create table if not exists dm_aifox.d_2level as 
  select
    asset_item_no,
    date(create_time) as dt,
    due_diff_days,
...
create table if not exists dm_aifox.day_detial_jf as
select
a.user_id,
a.asset_item_no,
a.user_status,
...
create table if not exists dm_aifox.tha_asset_c_card_2_jf as
-- 汇总
select
"金额" as stat_type,
'D-2' as overdue_type,
