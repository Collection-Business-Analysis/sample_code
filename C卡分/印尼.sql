...
with d3level as (
  ...
  from hive.rpt.rpt_w_cscore_model_result_export
  where etl_time >= '2024-10-01'
  and due_diff_days = 3
  group by 1,2,3,4,5)
, d1level as (
  select
...
create table if not exists dm_aifox.ine_asset_c_card_2_jf as
...
sum(coalesce(max_D_3_score,0)) as C_score,
